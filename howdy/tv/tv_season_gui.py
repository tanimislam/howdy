...
from howdy.core import core, QDialogWithPrinting, QLabelWithSave
from howdy.core import get_formatted_size, get_formatted_duration
from howdy.movie import movie

class HowdyTVSeasonGUI( QDialogWithPrinting ):
    ...
    def __init__( self, seriesName, seasno,
        ...
        super( HowdyTVSeasonGUI, self ).__init__( parent, isIsolated = False, doQuit = False )
        self.hide( )
        self.setModal( True )
        self.parent = parent
        ...
        #
        ## fill out those episodes don't have, use HowdyTV and TMDB stuff
        ## ONLY on missing episodes
        missing_eps = HowdyTVSeasonGUI.find_missing_eps( toGet, seriesName, seasno )
        series_id = tv.get_series_id( seriesName, tvdb_token, verify = verify )
        eps = tv.get_episodes_series(
        ...
        if seasonPICURL is not None:
            try:
                self.picData = core.get_pic_data(
            ...
            if self.picData is None and 'tvdbid' in plex_tv_data[ seriesName ]:
                imgURL, status = tv.get_series_season_image(
        ...
        self.seasonSummaryArea.setHtml(
            HowdyTVSeasonGUI.processSeasonSummary( seasno, episodes ) )
        ...
        self.tm = HowdyTVSeasonTableModel( self, episodes )
        self.tv = HowdyTVSeasonTableView( self )
        #
...
class HowdyTVSeasonTableView( QTableView ):
    def __init__( self, parent ):
        super( HowdyTVSeasonTableView, self ).__init__( parent )
        self.parent = parent
        self.proxy = HowdyTVSeasonQSortFilterProxyModel(
            self, self.parent.tm )
...
class HowdyTVSeasonQSortFilterProxyModel( QSortFilterProxyModel ):
    def __init__( self, parent, model ):
        super( HowdyTVSeasonQSortFilterProxyModel, self ).__init__( parent )
...
class HowdyTVSeasonTableModel( QAbstractTableModel ):
    ...
    def __init__( self, parent, episodes ):
        super( HowdyTVSeasonTableModel, self ).__init__( parent )
        self.parent = parent
