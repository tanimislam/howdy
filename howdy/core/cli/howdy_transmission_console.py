...
#
from howdy.core import core_transmission, core_deluge

def _get_matching_torrents( client, list_of_torrents, operation_if_size_1 = False ):
    if any(map(lambda tok: tok == "*", list_of_torrents ) ):
        return core_transmission.transmission_get_matching_torrents( client, [ "*" ] )
    if operation_if_size_1 and len( list_of_torrents ) == 0:
        return core_transmission.transmission_get_matching_torrents( client, [ "*" ] )
    return core_transmission.transmission_get_matching_torrents( client, list_of_torrents )
...
def main( ):
    ...
    parser_info.add_argument( 'info_torrent', metavar='torrent', type=str, nargs='*', # can be even ZERO arguments
                             help = ' '.join([
                                 ...
                                 'Example usage is "howdy_transmission_console info ab1 bc2", where "ab1" and "bc2" are the first three digits of the MD5 hashes of torrents to examine.' ]))
    parser_info.add_argument( '-f', '--file', dest='info_do_filename', action='store_true', default = False,
                             help = 'If chosen, then spit out the torrent selections into a debug output file. Name of the file is given by howdy_transmission_console.YYYYMMDD-HHMMSS.txt' )
    #
    ## resume
    ...
    parser_resume.add_argument( 'resume_torrent', metavar='torrent', type=str, nargs='+',
                               help = ' '.join([
                                   ...
                                   'Example usage is "howdy_transmission_console resume ab1 bc2", where "ab1" and "bc2" are the first three digits of the MD5 hashes of torrents to resume.' ]))
    #
    ## pause
    ...
    parser_pause.add_argument( 'pause_torrent', metavar = 'torrent', type=str, nargs='+',
                              help = '\n'.join([
                                  ...
                                  'Example usage is "howdy_transmission_console resume ab1 bc2", where "ab1" and "bc2" are the first three digits of the MD5 hashes of torrents to pause.' ]))
    #
    ## remove/delete
    ...
    if status != 'SUCCESS':
        print( "ERROR, COULD NOT GET VALID TRANSMISSION CLIENT." )
        return
    ...
    if args.choose_option == 'info':
        ...
        torrentInfo = core_transmission.transmission_get_torrents_info( client )
        ...
        if args.info_do_filename:
            fname = 'howdy_transmission_console.%s.txt' % (
    ...
    if args.choose_option == 'resume':
        ...
        core_transmission.transmission_resume_torrent( client, torrentIds )
        return
    if args.choose_option == 'pause':
        pause_torrents = args.pause_torrent
        torrentIds = _get_matching_torrents( client, pause_torrents )
        core_transmission.transmission_pause_torrent( client, torrentIds )
        return
    if args.choose_option in ( 'rm', 'del' ):
        remove_torrents = args.remove_torrent
        torrentIds = core_transmission.transmission_get_matching_torrents( client, remove_torrents )
        core_transmission.transmission_remove_torrent( client, torrentIds, remove_data = args.remove_data )
        return
