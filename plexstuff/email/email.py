...
from plexstuff.email import get_email_service, send_email_lowlevel, send_email_localsmtp, emailAddress, emailName
#
...
def get_summary_body( token, nameSection = False, fullURL = 'http://localhost:32400', verify = True ):
    ...
    .. seealso::
       ...
       * :py:meth:`get_summary_data_music_remote <plexstuff.email.email.get_summary_data_music_remote>`
       * :py:meth:`get_summary_data_movies_remote <plexstuff.email.email.get_summary_data_movies_remote>`
       * :py:meth:`get_summary_data_television_remote <plexstuff.email.email.get_summary_data_television_remote>`
    ...
    wholestr = open( os.path.join( resourceDir, 'plexstuff_body_template.tex' ), 'r' ).read( )
    wholestr = wholestr % tup_formatting
    if nameSection:
...
def send_individual_email_full(
    ...
    """
    Sends the HTML email, with optional *single* attachment, to a single recipient email address, using the `GMail API`_. Unlike :py:meth:`send_individual_email_full_withsingleattach <plexstuff.email.email.send_individual_email_full_withsingleattach>`, the attachment type is *also* set.
...
def send_individual_email_full_withattachs(
    ...
    """
    Sends the HTML email, with optional attachments, to a single recipient email address. This uses the :py:class:`SMTP <smtplib.SMTP>` Python functionality to send through a local SMTP_ server (see :py:meth:`send_email_localsmtp <plexstuff.email.send_email_localsmtp>`).
...
def get_summary_html( token, fullURL = 'http://localhost:32400',
    ...
    :param dict pngDataDict: optional argument. A :py:class:`dict` of PNG images stored in the Imgur_ main album. The structure of this dictionary is described in :py:meth:`processValidHTMLWithPNG <plexstuff.plexcore.plexcore.processValidHTMLWithPNG>`. Default is ``{}``.
    :param str name: optional argument. If given, the recipient's name.

    :returns: an HTML :py:class:`string <str>` document of the Plex_ newletter email.
    :rtype: str
    
    .. seealso:: :py:meth:`get_summary_body <plexstuff.email.email.get_summary_body>`
    """
    nameSection = False
    ...
    wholestr = open( os.path.join( resourceDir, 'plexstuff_template.tex' ), 'r' ).read( )
    wholestr = wholestr % tup_formatting
