...
This document describes the lower level Plexstuff TVDB API, upon which the :ref:`command line tools <TVDB Command Line Utilities>` and  the :ref:`GUI tool <plex_tvdb_gui>` are built. It lives in ``plexstuff.plextvdb``.
