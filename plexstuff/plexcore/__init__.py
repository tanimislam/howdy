...
def get_lastupdated_string( dt = datetime.datetime.now( ) ):
    ...
    .. seealso:: :py:meth:`get_summary_body <plexstuff.plexemail.plexemail.get_summary_body>`
    """
...
class QLabelWithSave( PyQt5.QtWidgets.QLabel ):
    ...
    def screenGrab( self ):
        ...
        .. seealso:: :py:meth:`QDialogWithPrinting.screenGrab <plexstuff.plexcore.QDialogWithPrinting.screenGrab>`
        """
...
class QDialogWithPrinting( PyQt5.QtWidgets.QDialog ):
    ...
    def screenGrab( self ):
        ...
        .. seealso:: :py:meth:`QLabelWithSave.screenGrab <plexstuff.plexcore.QLabelWithSave.screenGrab>`
        """
        fname, _ = QFileDialog.getSaveFileName(
...
class ProgressDialogThread( PyQt5.QtCore.QThread ):
    ...
    * It must end with this command, ``self.stopDialog.emit( )`` to hide the progress dialog widget.
    ...
    In the :py:meth:`run <plexstuff.ProgressDialogThread.run>` method, if one wants to print out something into ``progess_dialog``, then there should be these types of commands in ``run``: ``self.emitString.emit( mystr )``, where ``mystr`` is a :py:class:`str` message to show in ``progress_dialog``, and ``emitString`` is a :py:class:`pyqtsignal <PyQt5.QtCore.pyqtSignal>` connected to the ``progress_dialog`` object's :py:meth:`addText( ) <plexstuff.plexcore.ProgressDialog.addText>`.
    ...
    :var stopDialog: the signal that is triggered to stop the ``progress_dialog``, calling :py:meth:`stopDialog <plexstuff.plexcore.ProgressDialog.stopDialog>`.
    :var startDialog: the signal, with :py:class:`str` signature, that is triggered to restart the ``progress_dialog`` widget, calling :py:class:`startDialog <plexstuff.plexcore.ProgressDialog.startDialog>`.
    ...
    :type progress_dialog: :py:class:`ProgressDialog <plexstuff.plexcore.ProgressDialog>`
...
def splitall( path_init ):
    """
    This routine is used by :func:`get_path_data_on_tvshow <plexstuff.plextvdb.plextvdb.get_path_data_on_tvshow>` to split a TV show file path into separate directory delimited tokens.
