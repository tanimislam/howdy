...
#
from plexstuff.plexcore import plexcore_deluge, get_formatted_size, get_maximum_matchval, return_error_raw, plexcore
from plexstuff.plextvdb import get_token, plextvdb

_num_to_quit = 10

def return_error_couldnotfind( name ):
    """
    Returns a :py:class:`tuple` error message in the format of :py:meth:`return_error_raw <plexstuff.plexcore.return_error_raw>`, but with the message ``"Failure, could not find any tv shows with search term <name>"``, where ``<name>`` is the episode torrent that could not be found.

    :param str name: the episode torrent to be searched.
    
    :returns: a :py:class:`tuple` of format :py:meth:`return_error_raw <plexstuff.plexcore.return_error_raw>`, with the error message, ``"Failure, could not find any tv shows with search term <name>"``.
    :rtype: str
    
    .. seealso:: :py:meth:`return_error_raw <plexstuff.plexcore.return_error_raw>`
    """
...
def get_tv_torrent_eztv_io( name, maxnum = 10, verify = True, series_name = None,
    ...
    If this is unsuccessful, then returns an error :py:class:`tuple` of the form returned by :py:meth:`return_error_raw <plexstuff.plexcore.return_error_raw>`.
...
def get_tv_torrent_jackett( name, maxnum = 10, minsizes = None, maxsizes = None, keywords = [ ],
    ...
    import validators
    assert( maxnum >= 5 )
    data = plexcore.get_jackett_credentials( )
    if data is None:
