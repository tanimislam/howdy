...
This document describes the lower level Plexstuff TVDB API, upon which the :ref:`command line tools <TVDB Command Line Utilities>` and  the :ref:`GUI tool <plex_tvdb_totgui>` are built. It lives in ``plexstuff.plextvdb``.
