...
def plexapi_music_playlist_info( plex_playlist ):
    ...
    * ``album year``
    ...
    all_valid_tracks = sorted(
        filter( None, map( _get_info_playlist_item, enumerate( plex_playlist.items( ) ) ) ),
        key = lambda entry: entry[ 'order in playlist' ] )
    valid_columns = [
        'order in playlist', 'filename', 'added date', 'song name',
        'artist', 'track number', 'album', 'album number of tracks', 'album year' ]
    df = pandas.DataFrame(
        dict(map(lambda column: ( column, list(map(lambda entry: entry[ column ], all_valid_tracks ) ) ),
                valid_columns ) ) )
    logging.info( 'FOUND %d / %d VALID TRACKS IN PLAYLIST = %s IN %0.3f SECONDS.' % (
        len( all_valid_tracks ), len( plex_playlist.items( ) ), plex_playlist.title, time.perf_counter( ) - time0 ) )
    #return df.sort_values( 'added date' )
    return df
