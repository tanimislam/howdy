...
class TVShow( object ):
    ...
    :var dict seasonDict: a :py:class:`dict`, whose keys are the season numbers and whose values are the :py:class:`TVSeason <howdy.tv.tv.TVSeason>` associated with that season of the series.
    ...
    def get_tot_epdict_tvdb( self ):
        ...
        * The second level dictionary has keys (for each season) that are the season's episodes. Each value is a :py:class:`tuple` of episode name and air date, as a :py:class:`date <datetime.date>`.
        ...
        An example of the output format is described in the pure method :py:meth:`get_tot_epdict_tvdb <howdy.tv.tv.get_tot_epdict_tvdb>`.
...
class TVSeason( object ):
    ...
    :param bool verify: optional argument, whether to verify SSL connections. Default is ``True``.
    :param list eps: optional argument. The :py:class:`list` of TV shows returned by the TVDB_ API, and whose format is described in :py:meth:`get_episodes_series <howdy.tv.tv.get_episodes_series>`.
...
def get_tvdata_ordered_by_date( tvdata ):
    """
    This *flattens* the TV data, stored as a nested :py:class:`dict` (see documentation in :py:meth:`get_library_data <howdy.core.core.get_library_data>` that focuses on the format of the dictionary for TV libraries) into a dictionary whose keys are the :py:class:`date <datetime.date>` at which episodes have been aired, and whose values are the :py:class:`list` of episodes aired on that date. Each element of that list is a :py:class:`tuple` representing a given episode: ``( SHOW, SEASON #, EPISODE #, EPISODE TITLE )``.

    For example, let ``tvdata`` represent the Plex_ TV library information  collected with :py:meth:`get_library_data <howdy.core.core.get_library_data>`. Then here are the episodes on the Plex_ server aired on the *last* date.
...
def get_possible_ids( series_name, tvdb_token, verify = True ):
    """
    Returns a :py:class:`tuple` of candidate TV shows from the TVDB_ database that match a series name, and status string. If matches could not be found, returns an error :py:class:`tuple` returned by :py:meth:`return_error_raw <howdy.core.return_error_raw>` -- first element is ``None``, second element is :py:class:`str` error message.
    ...
    :returns: a :py:class:`tuple` of candidate TVDB_ series that match ``series_name``, and the :py:class:`str` ``'SUCCESS'``. Otherwise an error tuple returned by :py:meth:`return_error_raw <howdy.core.return_error_raw>`.
    :rtype: tuple
    ...
    tmdb_ids = movie.get_tv_ids_by_series_name( series_name, verify = verify )
    imdb_ids = list(filter(None, map(lambda tmdb_id: movie.get_tv_imdbid_by_id(
...
def get_all_series_didend(
    ...
    :param bool verify: optional argument, whether to verify SSL connections. Default is ``True``.
    :param int num_threads: the number of threads over which to parallelize this calculation. The default is *twice* the number of cores on the CPU.
    :param str tvdb_token: optional TVDB_ API access token. If ``None``, then gets the TVDB_ API access token with :py:meth:`get_token <howdy.tv.get_token>`.
...
def get_remaining_episodes(
    ...
    * the remaining keys -- ``prefix``, ``showFilename``, ``min_inferred_length``, ``season_prefix_dict``, ``episode_number_length``, ``avg_length_mins`` -- and their values have the same meaning as the summary Plex_ TV library returned by :py:meth:`get_path_data_on_tvshow <howdy.tv.tv.get_path_data_on_tvshow>`.
    ...
    .. seealso::
       ...
       * :py:meth:`get_path_data_on_tvshow <howdy.tv.tv.get_path_data_on_tvshow>`.
       * :ref:`get_tv_batch`.
    ...
    # for tvshow in toGet:
    #     info = toGet[ tvshow ]
    #     epdicts_tmdb = tv_attic.get_tot_epdict_tmdb( tvshow )
    #     #
...
def get_shows_to_exclude( tvdata = None ):
    ...
    :param dict tvdata: Optional Plex_ TV library information, returned by :py:meth:`get_library_data <howdy.core.core.get_library_data>`, to query. If not defined, then return the list of excluded TV shows found in the relevant table. If defined, returns an intersection of shows found in the table with TV shows in the Plex_ library.
...
def download_batched_tvtorrent_shows( tvTorUnits, newdirs = [ ], maxtime_in_secs = 240, num_iters = 10,
    ...
    :param list newdirs: the :py:class:`list` of new directories to create for the TV library. This is the second element of the :py:class:`tuple` returned by :py:meth:`create_tvTorUnits <howdy.tv.tv.create_tvTorUnits>`.
