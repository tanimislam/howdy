...
#
from howdy.movie import movie, movie_torrents
from howdy.core import core, get_popularity_color, get_formatted_size_MB, core_deluge
from howdy.core import QDialogWithPrinting, ProgressDialog
from howdy.email import email
...
class HowdyRefreshMoviesThread( QThread ):
    emitString = pyqtSignal( str )
    endRun = pyqtSignal( )

    def __init__( self, tmdbg ):
        super( HowdyRefreshMoviesThread, self ).__init__( )
        self.tmdbg = tmdbg
...
class HowdyMovieGUIFilloutCalculationThread( QThread ):
    emitString = pyqtSignal( str )
    endRun = pyqtSignal( )

    def __init__( self, tm, status, tup ):
        super( HowdyMovieGUIFilloutCalculationThread, self ).__init__( )
...
class HowdyMovieGUIThread( QThread ):
    emitString = pyqtSignal( str )
    endRun = pyqtSignal( )

    def __init__( self, void_func_dict ):
        super( HowdyMovieGUIThread, self ).__init__( )
...
class HowdyMovieMovieInfo( QDialogWithPrinting ):
    
    def __init__( self, parent, datum, verify = True ):
        super( HowdyMovieMovieInfo, self ).__init__(
        ...
        if self.tmdb_id is not None:
            self.cast_and_crew = movie.get_cast_and_crew(
...
class HowdyMovieTorrents( QDialogWithPrinting ):
    
    class HowdyMovieTorrentsTableView( QTableView ):
        def __init__( self, parent, torrentStatus ):
            super( HowdyMovieTorrents.HowdyMovieTorrentsTableView,
    ...
    class HowdyMovieTorrentsTableModel( QAbstractTableModel ):
        _columnNames = { 1 : [ 'title', 'seeds', 'leeches', 'size' ],
                         0 : [ 'title' ] }

        def __init__( self, parent, data_torrents, torrentStatus ):
            super( HowdyMovieTorrents.HowdyMovieTorrentsTableModel,
                   self ).__init__( parent )
            self.parent = parent
            self.torrentStatus = torrentStatus
            #
        ...
        def sendTorrentOrMagnetAtRow( self, row ):
            ...
            ## now send the email
            if self.torrentStatus == 0:
                status = email.send_email_movie_torrent(
                    jsondata[ 'movie' ], datum['content'], isJackett = False )
            elif self.torrentStatus == 1:
                status = email.send_email_movie_torrent(
                    jsondata[ 'movie' ], datum[ 'link' ], isJackett = True )
            else: status = email.send_email_movie_none( jsondata[ 'movie' ] )
            #
            if status != 'SUCCESS':
    ...
    def _createHowdyMovieTorrentsTableModel(
        ...
        if status == 'SUCCESS': # have torrent files
            ...
            return HowdyMovieTorrents.HowdyMovieTorrentsTableModel(
        ...
        #
        if len( data ) == 0:
            return HowdyMovieTorrents.HowdyMovieTorrentsTableModel( self, [ ], -1 )
        ...
        if len( data_torrents ) == 0:
            return HowdyMovieTorrents.HowdyMovieTorrentsTableModel( self, [ ], 0 )
        return HowdyMovieTorrents.HowdyMovieTorrentsTableModel( self, data_torrents, 1 )

    def __init__( self, parent, token, movie_name, tmdb_id,
                  bypass = False, maxnum = 10, useIMDB = False,
                  do_debug = False ):
        super( HowdyMovieTorrents, self ).__init__(
        ...
        self.tmdbTorrentModel = self._createHowdyMovieTorrentsTableModel(
        ...
        if self.tmdbTorrentModel.torrentStatus == -1:
            ...
            self.tmdbTorrentView = HowdyMovieTorrents.HowdyMovieTorrentsTableView(
...
class HowdyMovieGUI( QDialogWithPrinting ):
    ...
    def __init__( self, token, fullURL, movie_data_rows,
        ...
        super( HowdyMovieGUI, self ).__init__(
        ...
        self.tmdbtv = HowdyMovieTableView( self )
    ...
    def _connectHowdyMovieTableView( self ):
    ...
    def getChosenMovies( self ):
        ...
        initThread = HowdyMovieGUIFilloutCalculationThread(
    ...
    def refreshMovies( self ):
        ...
        progress_dialog.startDialog( )
        initThread = HowdyRefreshMoviesThread( self )
        initThread.emitString.connect( progress_dialog.addText )
        initThread.endRun.connect( progress_dialog.stopDialog )
        initThread.start( )
        progress_dialog.exec_( )
...
class SelectYearGenreWidget( QWidget ):
    ...
    def __init__( self, parent ):
        ...
        self.genreComboBox.addItems( sorted( movie.TMDBEngine( parent.verify ).getGenres( ) ) )
    ...
    def returnSYG( self ):
        ...
        if state == 0:
            ...
            genre_id = movie.TMDBEngine( self.parent.verify ).getGenreIdFromGenre( genre )
...
class HowdyMovieTableView( QTableView ):
    def __init__( self, parent ):
        super(HowdyMovieTableView, self).__init__( parent )
        #
        self.parent = parent
        self.token = parent.token
        self.tm = HowdyMovieTableModel( self, parent.verify )
        self.proxy = HowdyMovieQSortFilterProxyModel( self, self.tm )
...
class HowdyMovieQSortFilterProxyModel( QSortFilterProxyModel ):
    def __init__( self, parent, tm ):
        super(HowdyMovieQSortFilterProxyModel, self).__init__( parent )
        #
...
class HowdyMovieTableModel( QAbstractTableModel ):
    ...
    def __init__( self, parent = None, verify = True ):
        super(HowdyMovieTableModel, self).__init__( parent )
        self.parent = parent
        self.verify = verify
    ...
    def infoOnMovieAtRow( self, actualRow ):
        ...
        tmdbmi = HowdyMovieMovieInfo( self.parent, datum, verify = self.verify )
        result = tmdbmi.exec_( )
    ...
    #
    ## three ways that what can be displayed in HowdyMovieTableModel can be changed
    ...
    def fillOutCalculation( self, status, tup ):
        ...
        if status == 0:
            ...
            assert( genre_id in movie.TMDBEngine(
