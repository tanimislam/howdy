...
def signal_handler( signal, frame ):
    ...
    :param :py:class:`dict` signal: the POSIX signal to capture. See `the Python 3 signal high level overview <signal_high_level_overview_>`_ to begin to understand what POSIX signals are, and how Python can expose functionality to interact with them.
    :param int frame: the stack frame. I don't know what it is, or why it's necessary in this context, when trying to capture a ``Ctrl+C`` and cleanly exit.
...
# :param :py:class:`Signals <signal.Signals>` signal: the UNIX (or POSIX?) signal to capture.

# :param :py:class:`frame` frame: the stack frame. I don't know what it is, or why it's necessary in this context, when trying to capture a ``Ctrl+C`` and cleanly exit.
