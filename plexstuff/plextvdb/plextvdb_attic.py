...
def get_tot_epdict_omdb( showName, apikey, inYear = None ):
    ...
    .. seealso::
       ...
       * :py:meth:`get_tot_epdict_imdb <plexstuff.plextvdb.plextvdb_attic.get_tot_epdict_imdb>`.
       * :py:meth:`get_tot_epdict_tmdb <plexstuff.plextvdb.plextvdb_attic.get_tot_epdict_tmdb>`.
    """
    if inYear is None:
...
def get_tot_epdict_tmdb( showName, firstAiredYear = None, showSpecials = False ):
    """
    Returns a :py:class:`dict` of episodes found from the TMDB_ API. The top level dictionary's keys are the season numbers, and each value is the next-level dictionary of season information. The next level, season dictionary's keys are the episode number, and its values are a two-element :tuple: of episode names and aired dates (as a :py:class:`date <datetime.date>` object). This two level dictionary has the same format as the output from :py:meth:`get_tot_epdict_tvdb <plexstuff.plextvdb.plextvdb.get_tot_epdict_tvdb>`. For example, for `The Simpsons`_,
    ...
    .. seealso::
       ...
       * :py:meth:`get_tot_epdict_tvdb <plexstuff.plextvdb.plextvdb.get_tot_epdict_tvdb>`.
       * :py:meth:`get_tot_epdict_imdb <plexstuff.plextvdb.plextvdb_attic.get_tot_epdict_imdb>`.
       * :py:meth:`get_tot_epdict_omdb <plexstuff.plextvdb.plextvdb_attic.get_tot_epdict_omdb>`.
    """
...
def get_tot_epdict_imdb( showName, verify = True ):
    ...
    * The second level dictionary has keys (for each season) that are the season's episodes. Each value is a :py:class:`tuple` of episode name and air date, as a :py:class:`date <datetime.date>`.
    ...
    An example of the structure of this dictionary can be found in :py:meth:`get_tot_epdict_tvdb <plexstuff.plextvdb.plextvdb.get_tot_epdict_tvdb>`.
