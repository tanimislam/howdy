...
from plexstuff.plexcore import plexcore, baseConfDir, session, PlexConfig
from plexstuff.plexcore import return_error_raw, get_maximum_matchval
from plexstuff.plexmusic import pygn, parse_youtube_date, format_youtube_date
...
def oauth_store_google_credentials( credentials ):
    ...
    .. seealso:: :py:meth:`oauth_get_google_credentials <plexstuff.plexmusic.plexmusic.oauth_get_google_credentials>`
...
def oauth_get_google_credentials( ):
    ...
    .. seealso:: :py:meth:`oauth_store_google_credentials <plexstuff.plexmusic.plexmusic.oauth_store_google_credentials>`
    """
    val = session.query( PlexConfig ).filter( PlexConfig.service == 'gmusic_mobileclient' ).first( )
...
def gmusicmanager( useMobileclient = False, verify = True, device_id = None ):
    ...
    .. seealso:: :py:meth:`get_gmusicmanager <plexstuff.plexmusic.plexmusic.get_gmusicmanager>`
...
def get_gmusicmanager( useMobileclient = False, verify = True, device_id = None ):
    ...
    .. seealso:: :py:meth:`gmusicmanager <plexstuff.plexmusic.plexmusic.gmusicmanager>`
    """
...
def download_best_song( artist_name, song_name, youtube = None,
                        verify = True):
    """
    If successful, downloads the most likely YouTube_ clip into an M4A_ file, whose name is ``"artist_name"."song_name".m4a``. Uses the LastFM_ API functionality, through :py:class:`PlexLastFM <plexstuff.plexmusic.plexmusic.PlexLastFM>`, to get the music metadata.

    :param str artist_name: the artist name.
    :param str song_name: name of the song.
    :param Resource youtube: optional aegument, a `googleapiclient Resource`_ object that allows for the access to the `YouTube Google API`_. If not defined, then instantiated with :py:meth:`get_youtube_service <plexstuff.plexmusic.plexmusic.get_youtube_service>`.
    :param bool verify: optional argument, whether to verify SSL connections. Default is ``True``.
...
class PlexLastFM( object ):
    ...
    :param dict data: optional argument, containg the LastFM_ API data: ``api_key``, ``api_secret``, ``application_name``, and ``username``. See :numref:`The Gracenote and LastFM APIs` to understand how to set up the LastFM_ API credentials. If not given, then gets the LastFM_ API data from :py:meth:`PlexLastFM.get_lastfm_credentials <plexstuff.plexmusic.plexmusic.PlexLastFM.get_lastfm_credentials>`.
    :param bool verify: optional argument, whether to verify SSL connections. Default is ``True``.
    ...
    def get_music_metadata( self, song_name, artist_name, all_data = False ):
        """
        Uses the MusicBrainz_ API to fill out as much metadata as possible for a given song. Before running this method, one should first set the MusicBrainz_ API header data with :py:meth:`MusicInfo.get_set_musicbrainz_useragent <plexstuff.plexmusic.plexmusic.`MusicInfo.get_set_musicbrainz_useragent>`.
...
class PlexMusic( object ):
    """
    This object uses the Gracenote_ API to get information on songs, albums, and artists. Uses :py:meth:`PlexMusic.get_gracenote_credentials <plexstuff.plexmusic.plexmusic.PlexMusic.get_gracenote_credentials>` to get the Gracenote_ API configuration data.
